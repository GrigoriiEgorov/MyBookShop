    ...
    public String saveNewBookImage(MultipartFile file, String slug) throws IOException {
        ...
        if(!file.isEmpty()){
            if(!new File(uploadPath).exists()){
                ...
                Logger.getLogger(ResourceStorage.class.getSimpleName()).info("create image folder in " + uploadPath);
            }
            String fileName = slug + "." + FilenameUtils.getExtension(file.getOriginalFilename());
            Path path = Paths.get(uploadPath,fileName);
            resourceURI = "/book-covers/" + fileName;
            file.transferTo(path);
            Logger.getLogger(ResourceStorage.class.getSimpleName()).info("save image in " + uploadPath);
        }
        return resourceURI;
    }
